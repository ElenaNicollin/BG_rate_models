...
with PdfPages(f"outputs/rate/{args.outfile}.pdf") as pdf:
    for s, species in enumerate(species_list):
        ...
        for i_loop, (loop_name, loop_params) in enumerate(loops.items()):
            ...
            ax[1].set_xlim(0,60)
            plot_relative_phase_angles(rates, pops_sublist, "Proto", dt, skip=skip, loop_name=loop_name, normalize=True, n_pairs=2000, ax=ax[2])
            plot_relative_phase_angles(rates, pops_sublist, "Proto", dt, skip=skip, loop_name=loop_name, normalize=True, n_pairs=2000, ax=ax_main[i_loop, s])
            # plt.show()
